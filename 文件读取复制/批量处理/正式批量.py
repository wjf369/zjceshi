...
def chuancan(can1,can3,can4):
    ...
    if can4 - can3 > 1001:
        print("超过1000条")
        print(can4 - can3)
        file = open('超过1000条2月4号.txt', 'a', encoding='UTF-8')
        file.write(str(can1))
        file.write("\r\n")
        file.close()
    elif rows2 == []:
        print("机构不存在")
        file = open('机构不存在2月4号.txt', 'a', encoding='UTF-8')
        file.write(str(can1))
        file.write("\r\n")
        file.close()
    else:
        ...
        while leng >= 0:
            ...
            for row1 in rows1:
                if len(row1[0]):
                    ...
                    file = open('有数据2月4号.txt', 'a', encoding='UTF-8')
                    ...
                    mydb.commit()
                    file = open('增加成功2月4号.txt', 'a', encoding='UTF-8')
                    file.write("\r\n")
                    file.write(str(bian3))
                    file.write("\r\n")
                    file.close()
