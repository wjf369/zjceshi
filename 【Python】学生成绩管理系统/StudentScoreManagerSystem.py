...
class PyMySQLUtils:
    ...
    def __init__(self):
        # 打开数据库连接
        self.db = pymysql.connect("127.0.0.1", "root", "123456", "test1")
        # 使用cursor()方法获取操作游标
...
class TeacherMenu:
    def __init__(self, parent_window):
        ...
        Button(self.frame_right, text="写入到Excel文件", width=30,
               command=lambda: mysql_excel("D:\\pythonProject\\【Python】学生成绩管理系统\\StudentScore.xls")).grid(row=5, column=0, padx=5, pady=5)
...
class prepare:
    ...
    def excel_mysql(self):
        book = xlrd.open_workbook("D:\\pythonProject\\【Python】学生成绩管理系统\\NewStudentScore.xls")
